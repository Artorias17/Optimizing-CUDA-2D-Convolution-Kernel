...
BenchmarkResult benchmark_naive_conv(const float *d_input,
    ...
    if (warmup_runs < 0 || timed_runs <= 0) {
        std::cerr << "Invalid benchmark run counts." << std::endl;
    ...
    const float total_time =
        std::accumulate(execution_times.begin(), execution_times.end(), 0.0f);

    const float mean_ms = total_time / static_cast<float>(timed_runs);
    ...
    const float stddev_ms =
        std::sqrt(squared_difference_sum / static_cast<float>(timed_runs));

    const int filter_width = 2 * params.filter_radius + 1;

    const double output_pixels =
        static_cast<double>(params.H) * static_cast<double>(params.W);

    const double total_flops =
        2.0 * output_pixels * filter_width * filter_width;

    const double elapsed_seconds = static_cast<double>(mean_ms) / 1000.0;

    const float gflops =
        static_cast<float>(total_flops / elapsed_seconds / 1.0e9);

    const double estimated_bytes = output_pixels *
                                   (2.0 * filter_width * filter_width + 1.0) *
                                   sizeof(float);

    const float bandwidth_gbs =
        static_cast<float>(estimated_bytes / elapsed_seconds / 1.0e9);
...
BenchmarkResult benchmark_cudnn_conv(CudnnConvContext *context,
                                     ...
                                     int timed_runs) {
    if (context == nullptr) {
        std::cerr << "cuDNN benchmark context is null." << std::endl;
